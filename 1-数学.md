...
$\frac{1}{2n}\sum_{d|n}\phi(d)x_d^{n/d}+ \begin{cases}\frac{1}{2}x_1x_2^{\frac{n-1}{2}}\qquad & n是奇数 \\ \frac{1}{4}(x_2^{\frac{n}{2}}+x_1x_2^{\frac{n-2}{2}})\qquad & n是偶数 \end{cases}$
...
###### $C(n,m)=\frac{n^{m\over}}{m!}$，$n\in \R$

###### $(1-x)^{-(d+1)}=\sum_{n\geq 0}C(d+n,n)x^n$，$d\in \R$ 
