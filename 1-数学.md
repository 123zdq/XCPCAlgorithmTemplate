...
可以 $O(n)$ 求出 $1..n$ 模固定质数 $p$ 的逆元，但是空间局部性不佳，对于 $n=10^7$ 数量级最好用对阶乘求逆的 $O(n+log P)$ 做法
...
其中 $a_i\geq 0$ ，$m>0$ ，$2*lcm(m_1,...)\in long long$
...
\end{cases}$$
其中 $c=m的质因子最高次数 +1$ ，是一个大小为 $O(log\ n)$ 的常数，为计算方便常用$\phi(m)$代替
